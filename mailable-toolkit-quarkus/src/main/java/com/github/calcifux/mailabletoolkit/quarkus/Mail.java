package com.github.calcifux.mailabletoolkit.quarkus;
...
/**
 * Framework-agnostic static facade for sending mail — the portable twin of the Spring starter's
 * {@code com.github.calcifux.mailabletoolkit.spring.Mail}, with the SAME signatures so application code
 * (e.g. the backend's {@code MailPocService}) compiles unchanged after swapping the import. The Quarkus
 * {@code MailCdiService} wires the {@link Mailer} and (optionally) the {@link MailQueue} into it on the
 * standard CDI {@code @Initialized(ApplicationScoped.class)} startup event.
 *
 * <pre>{@code
 * Mail.send(mailable, List.of(email));                              // sync
 * Mail.queue(mailable, List.of(email));                            // async (mailable's queue, or default)
 * Mail.mailer("billing").send(mailable, List.of(email));           // pick the SMTP
 * Mail.onQueue("priority").queue(mailable, List.of(email));        // pick the queue
 * String html = Mail.preview(mailable);                            // render only, no send
 * }</pre>
 */
public final class Mail {
    ...
    /** Wired once by {@code MailCdiService} on startup. */
    public static void init(Mailer mailer, MailQueue queue, String defaultQueue) {
        Mail.mailer = mailer;
        Mail.queue = queue;
    ...
    private static Mailer require() {
        if (mailer == null) {
            throw new MailToolkitException("Mail facade not initialized — is mailable-toolkit-quarkus on the classpath and MailCdiService started?");
        }
        return mailer;
    }
